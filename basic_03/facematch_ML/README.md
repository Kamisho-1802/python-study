...
現在の進捗: **フェーズ3まで**（検索ロジック + FastAPI API）。
...
フェーズ2でインデックスを作ったうえで、API を起動する（プロジェクト直下から）:

```bash
uvicorn src.main:app --reload --port 8000
```

ブラウザで `http://localhost:8000/docs` を開き、`/analyze` に自分の写真を
アップロードすると、似ている有名人 top3 が類似度付きの JSON で返る。

- 顔のない画像・複数人の画像では `{"error": "..."}` が返る。
- 撮影画像は一時ファイルで処理し、サーバに保存しない（第11章）。
- `--reload` は開発用。本番では付けない。
